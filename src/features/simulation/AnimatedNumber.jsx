...
export function DonutPercent({ percent = 0, size = 120, stroke = 12, color = '#34d399', bg = '#e6eef6' }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <g transform={`translate(${size/2},${size/2})`}>
        ...
        <text x="0" y="6" textAnchor="middle" fontSize="18" fill="#0f172a" fontWeight="700">{Math.round(percent)}%</text>
      </g>
    </svg>
  );
}
